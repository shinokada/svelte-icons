	...
	<Card img={img['src']}>
		<div class="m-6">
			...
			<h5 class={hFiveClass}>
				...
				{#if a11y}
					<Badge {href} target="_blank" class="absolute top-1 right-0 text-sm dark:text-white"
						>A11y</Badge
					>
				{/if}
			</h5>
...
<div class="flex flex-wrap justify-center gap-6">
	{#each svelte5_icons_dynamic as { packageName, label, link, link2, logo, logoColor, labelColor, img, download_color, version_color, desc, source, repo, docs, tag, a11y, href } (packageName)}
		{@render runesIcon({
			packageName,
			label,
			link,
...
<div class="flex flex-wrap justify-center gap-6">
	{#each illust as { packageName, label, link, link2, logo, logoColor, labelColor, img, download_color, version_color, desc, source, repo, docs, tag, a11y, href } (packageName)}
		{@render runesIcon({
			packageName,
			label,
			link,
...
<div class="mb-8 flex flex-wrap justify-center gap-6">
	{#each svelte4_icons as { packageName, label, logo, logoColor, tag, link, link2, img, download_color, version_color, desc, source, repo, docs } (packageName)}
		{@render runesIcon({
			packageName,
			label,
			img,
